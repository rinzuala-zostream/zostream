@php
    ...
    $pageTitle = $ad->ads_name . ' — ZoStream Ads';
...
<head>
  ...
  <style>
    ...
    .card  { background: #121212; border-radius: 16px; box-shadow: 0 10px 24px rgba(0,0,0,.35); }
    .badge { background: #ffffff14; border: 1px solid #ffffff2a; }
    .hero::before { content:''; position:absolute; inset:0; background: radial-gradient(1200px 500px at 20% -10%, #ffffff1a, transparent 60%); pointer-events:none; }
  </style>
</head>
<body class="min-h-screen font-sans antialiased">
    ...
    <div class="relative max-w-6xl mx-auto px-4 sm:px-6 pt-12 pb-10">
      <div class="flex flex-col md:flex-row gap-6 items-start">
        ...
        <img src="{{ $preview }}"
             ...
             class="w-full md:w-[320px] h-[220px] md:h-[260px] object-cover rounded-2xl shadow-2xl ring-1 ring-white/10" alt="Cover">

        {{-- Title + Meta --}}
        <div class="flex-1">
          <div class="flex flex-wrap items-center gap-3 mb-3">
            <h1 class="text-3xl md:text-5xl font-extrabold tracking-tight">{{ $ad->ads_name }}</h1>
            <span class="text-xs md:text-sm px-2.5 py-1 rounded-md badge {{ $isActive ? 'text-emerald-300' : 'text-rose-300' }}">
          ...
          <div class="mt-5 flex flex-wrap items-center gap-2 md:gap-3 text-xs md:text-sm">
            ...
            <span class="px-3 py-1 rounded-full badge">Duration: {{ $period }} day{{ $period === 1 ? '' : 's' }}</span>
  ...
  <main class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 -mt-6 pb-14">
    ...
    {{-- Footer --}}
    <div class="text-center opacity-70 text-sm mt-6">
      Share link:
      <a class="underline decoration-dotted hover:decoration-solid" href="{{ $ad->ads_url }}">
        {{ $ad->ads_url }}
      </a>
    </div>
  </main>
  ...
  <div id="lightbox" class="fixed inset-0 hidden items-center justify-center bg-black/80 z-50 p-4">
    ...
    <div class="max-w-5xl w-full">
      <img id="lightboxImg" src="" class="w-full max-h-[80vh] object-contain rounded-xl ring-1 ring-white/10" alt="Preview">
