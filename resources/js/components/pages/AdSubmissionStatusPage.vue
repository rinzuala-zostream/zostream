...
const loading = ref(true); const saving = ref(false); const paying = ref(false); const error = ref(''); const notice = ref(''); const item = ref(null);
...
function headers(json = false) { return { Accept: 'application/json', ...(json ? { 'Content-Type': 'application/json' } : {}), 'X-Client-Platform': 'web', 'X-Client-Version': '1.0' }; }
...
async function load() {
    ...
    try { const response = await fetch(`/api/v4/ad-submissions/status/${encodeURIComponent(token)}`, { headers: headers() }); const payload = await response.json().catch(() => null); if (!response.ok || payload?.success === false) throw new Error(apiError(payload, 'Submission hmuh loh.')); item.value = payload.data; fillRevision(); }
    catch (reason) { error.value = reason.message || 'Submission status load theih lo.'; }
    finally { loading.value = false; }
}
async function resubmit() {
    saving.value = true; error.value = ''; notice.value = '';
    try { const response = await fetch(`/api/v4/ad-submissions/status/${encodeURIComponent(token)}/resubmit`, { method: 'POST', headers: headers(true), body: JSON.stringify(revision) }); const payload = await response.json().catch(() => null); if (!response.ok || payload?.success === false) throw new Error(apiError(payload, 'Resubmit theih lo.')); item.value = payload.data; notice.value = 'Revised details submit leh a ni.'; }
...
async function payInvoice() {
    ...
    try {
        await loadRazorpay();
        const response = await fetch(`/api/v4/ad-submissions/status/${encodeURIComponent(token)}/payments/razorpay/order`, { method: 'POST', headers: headers(true), body: '{}' });
        ...
        const checkout = new window.Razorpay({
            ...
            handler: async (payment) => {
                try {
                    const verify = await fetch(`/api/v4/ad-submissions/status/${encodeURIComponent(token)}/payments/razorpay/verify`, { method: 'POST', headers: headers(true), body: JSON.stringify(payment) });
...
<template>
    <main class="ad-status-page section-pad">
        <section class="ad-status-wrap">
            ...
            <div v-if="loading" class="ad-status-state">Loading submission…</div>
            <div v-else-if="error && !item" class="ad-status-state error"><h1>Status load theih lo</h1><p>{{ error }}</p></div>
            <template v-else-if="item">
                <header class="ad-status-header"><div><span>Submission status</span><h1>{{ item.reference_no }}</h1><p>{{ item.ads_name }} · {{ item.business_name }}</p></div><b :class="item.status">{{ label }}</b></header>
                ...
                <section class="ad-timeline"><h2>Timeline</h2><article v-for="event in item.events" :key="event.id"><i></i><div><b>{{ statusLabels[event.to_status] || event.action }}</b><p v-if="event.note">{{ event.note }}</p><time>{{ new Date(event.created_at).toLocaleString() }}</time></div></article></section>
            </template>
        </section>
    </main>
</template>
