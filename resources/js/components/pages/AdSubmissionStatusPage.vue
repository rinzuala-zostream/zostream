...
const loading = ref(Boolean(session.value)); const saving = ref(false); const paying = ref(false); const error = ref(''); const notice = ref(''); const item = ref(null);
...
async function load() {
    ...
    try { const response = await authenticatedFetch(`/api/v4/ad-submissions/status/${encodeURIComponent(token)}`, { headers: headers() }); const payload = await response.json().catch(() => null); if (response.status === 401) { clearPublicSession(); session.value = null; } if (!response.ok || payload?.success === false) throw new Error(apiError(payload, 'Submission hmuh loh emaw i account ta a ni lo.')); item.value = payload.data; fillRevision(); }
...
onMounted(() => { if (session.value) load(); });
</script>
    ...
    <main class="ad-status-page section-pad">
        <section class="ad-status-wrap">
            ...
            <template v-else-if="item">
                <header class="ad-status-header"><div><span>{{ isPaymentPage ? 'Ad payment' : 'Submission status' }}</span><h1>{{ item.reference_no }}</h1><p>{{ item.ads_name }} · {{ item.business_name }}</p></div><div class="ad-status-actions"><b :class="item.status">{{ label }}</b><button type="button" @click="logout">Logout</button></div></header>
