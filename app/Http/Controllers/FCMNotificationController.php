...
class FCMNotificationController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (!empty($token)) {
            return $this->sendToToken($token, $title, $body, $image, $key, is_array($data) ? $data : []);
        }

        return $this->sendToTopic($topic, $title, $body, $image, $key, is_array($data) ? $data : []);
    }

    public function sendToToken($token, $title, $body, $image = '', $key = null, array $data = [])
    {
        $accessToken = $this->getAccessToken($this->serviceAccountData());
    ...
    public function sendToTopic($topic, $title, $body, $image = '', $key = null, array $data = [])
    {
        $accessToken = $this->getAccessToken($this->serviceAccountData());
    ...
    private function serviceAccountData()
    {
        return [
