...
import { CreatePlanForm } from "@/app/features/subscriptions/components/create-plan-form";
  ...
  return (
    <main className="flex min-h-svh flex-1 flex-col px-3 py-1 text-slate-900 lg:h-svh lg:min-h-0 lg:overflow-hidden lg:px-2 lg:py-1 dark:text-white">
      <div className="flex w-full flex-1 flex-col lg:min-h-0">
        <AdminPageHeader title="Create plan" initialMode={initialMode} />
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                ...
                <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl dark:text-white">
                  Create pricing for one device type.
                </h2>
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
                  Add the plan price, stream limit, quality, and customer-facing
                  benefits. Feature rows are saved after the plan is created.
                </p>
              </div>
            </div>
          </section>
