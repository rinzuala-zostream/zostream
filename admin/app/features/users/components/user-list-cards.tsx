...
import { ChevronLeft, ChevronRight, PencilLine, UserPlus } from "lucide-react";
...
function userEditId(user: UserItem) {
...
function pageHref({
  ...
  return `/users/update?${params.toString()}`;
}
          ...
          <table className="min-w-[980px] w-full table-fixed border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-[rgba(15,23,42,0.1)] dark:divide-white/10">
              {users.map((user) => {
                ...
                const editId = userEditId(user);
                const active = valueToBoolean(user.isACActive);

                return (
                  <tr
                    key={editId || valueToString(user.mail)}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right align-middle">
                      <Link
                        href={`/users/update/${encodeURIComponent(editId)}`}
                        className={cn(
