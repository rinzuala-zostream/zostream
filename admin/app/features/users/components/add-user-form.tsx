...
import { Check, IdCard, Mail, MapPin, Save, UserPlus } from "lucide-react";
import { toast } from "react-toastify";
import {
  addUserAction,
  type AddUserFormState,
} from "@/app/(admin)/users/add/actions";
...
export function AddUserForm() {
  ...
  const statusMessage = useMemo(() => {
    if (state.status === "success" && state.uid) {
      return `${state.message} UID: ${state.uid}`;
    }

    return state.message;
  }, [state.message, state.status, state.uid]);
  ...
  useEffect(() => {
    ...
    if (state.status === "success") {
      toast.success(statusMessage || "User created.");
      return;
    }

    toast.error(statusMessage || "User could not be created.");
    ...
    <form ref={formRef} action={formAction} className="space-y-4 pb-28">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <label className="block min-w-0">
            ...
            <input
              name="uid"
              type="text"
              placeholder="Leave blank to use phone or email"
          ...
          <label className="block min-w-0">
            ...
            <input name="dob" type="date" className={inputClassName} />
          </label>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <label className="block min-w-0">
            ...
            <input
              name="device_name"
              type="text"
              placeholder="Android TV, mobile, browser"
              className={inputClassName}
            />
          </label>
          ...
          <label className="block min-w-0 lg:col-span-2">
            ...
            <input
              name="token"
              type="text"
              placeholder="Notification or auth token"
              className={inputClassName}
            />
          </label>
        </div>
      ...
      >
        <div className="flex items-center gap-3 text-sm font-semibold text-slate-700 dark:text-slate-200">
          ...
          <span>Create the user through the user service API.</span>
        </div>
        <button
          type="submit"
          disabled={isPending}
